...
**Responsável resolvido no momento do uso.** `TaskService.create` recusa um
`assigneeId` que não seja membro ativo, e um `defaultOwnerId` validado na
gravação pode ter sido suspenso depois. A execução resolve o responsável uma
vez (padrão configurado → criador do contrato → nenhum), aceitando só quem
ainda é membro ativo. Se ninguém serve, as tarefas nascem sem responsável —
tarefa sem dono é muito melhor que nenhuma tarefa.
...
O seletor de responsável padrão consome `GET /api/v1/team/directory`, do
módulo de gestão de equipe, e reusa o componente `MemberSelect` que leads e
tarefas já usam — a lista de gente é a mesma em todo o produto, e não há um
segundo endpoint de membros pra manter.
