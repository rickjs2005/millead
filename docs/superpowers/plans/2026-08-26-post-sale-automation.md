...
- Extensões: `ProjectChecklist` (+`leadId`/`contractId`/`startedAt`/`dueAt`),
  `Briefing` (+`contractId`), `MembershipRepository`
  (+`listMembersForOrg`/`isActiveMember`), `BriefingRepository`
  (+`findFirstByContractId`), `ProjectChecklistRepository` (+`findByContractId`).
...
Uma dependência transversal continua aberta: **gestão de equipe**. `ownerId`
(leads), `assigneeId` (tarefas) e o `defaultOwnerId` desta fase existem e
funcionam, mas com uma pessoa só na organização eles não mostram valor. O
`GET /settings/members` adicionado aqui é o primeiro tijolo dessa API.
