...
- [ ] **Gestão de membros (API + telas)**: continua sendo a única ausência
      de produto. Hoje `settings/team` é um `EmptyState` honesto; convidar
      alguém, trocar papel ou suspender acesso não existe. Sem isso a
      organização é de usuário único. **Primeiro tijolo já posto**: a
      automação pós-fechamento precisa escolher um responsável padrão, então
      existe um `GET /api/v1/settings/members` somente-leitura (id, nome,
      e-mail, papel). Falta o resto do CRUD e as telas.

- **Filtros por pessoa, ainda adormecidos**: `ownerId` (leads) já existe no
  schema e nos tipos, mas nenhum formulário o atribui e nenhuma tela filtra
  por ele — a coluna "Responsável" na tabela de leads exibe `—`. O
  `assigneeId` das tarefas **deixou de ser adormecido**: a automação
  pós-fechamento atribui o responsável padrão configurado a toda tarefa que
  cria. Ligar os filtros na UI vira trabalho de minutos quando a gestão de
  membros existir.
...
| Configurações | `GET /api/v1/settings/integrations`, `PATCH /profile`, `PATCH /organization`, `GET/PATCH /post-sale-automation`, `GET /members` |
