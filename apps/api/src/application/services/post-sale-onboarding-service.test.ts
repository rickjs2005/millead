...
interface HarnessOptions {
  ...
  /** false simula responsável configurado que foi suspenso depois. */
  ownerIsActiveMember?: boolean;
...
function makeHarness(options: HarnessOptions = {}): Harness {
  ...
  const deps: PostSaleOnboardingDeps = {
    ...
    } as unknown as PostSaleOnboardingDeps["memberships"],
    pipelines: {
...
describe("PostSaleOnboardingService — contratos fora do caso feliz", () => {
  ...
  it("responsavel configurado que foi suspenso nao derruba a etapa de tarefas", async () => {
    // TaskService.create recusa assigneeId que nao seja membro ativo. Tarefa
    // sem responsavel e muito melhor que nenhuma tarefa.
    const h = makeHarness({ ownerIsActiveMember: false });
    fullSettings(h.automation);

    const execution = await triggerAndRun(h);

    expect(execution.steps.find((s) => s.key === "TASKS")?.status).toBe("SUCCEEDED");
    expect(h.createdTasks.length).toBeGreaterThan(0);
    expect(h.createdTasks.every((t) => t.assigneeId === undefined)).toBe(true);
    // E a movimentacao do lead fica sem autor, em vez de apontar pra alguem
    // que nao esta mais na equipe.
    expect(h.movedStages[0]?.userId).toBeNull();
  });
