...
function stubPostSaleController() {
  return {
    ...
    listMembers: vi.fn(async (_req: Request, res: Response) => {
...
describe("rotas de configuração da automação pós-fechamento", () => {
  ...
  it("15c. usuário sem settings:manage não lista membros da organização", async () => {
    const { base, postSale } = await settingsApp(["leads:read"]);

    const res = await fetch(`${base}/api/v1/settings/members`);

    expect(res.status).toBe(403);
    expect(postSale.listMembers).not.toHaveBeenCalled();
  });
